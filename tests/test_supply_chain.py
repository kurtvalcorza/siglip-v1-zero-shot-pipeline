...
import siglip_v1_pipeline.model as model_module
from siglip_v1_pipeline.config import (
...
def test_model_identity_and_revision_are_explicit():
    assert MODEL_ID == "google/siglip-base-patch16-256"
...
def test_verify_checkpoint_checks_size_and_digest(
    ...
    payload = b"verified-siglip-v1-test-payload"
...
def test_verify_checkpoint_validates_manifest_and_detects_tampered_preprocessor(
    ...
    manifest = {
        ...
        "formatVersion": 1,
        "modelKey": "siglip-base-patch16-256",
        "files": [
            {
                "path": MODEL_FILENAME,
                "bytes": len(payload),
...
def test_resolve_weights_path_precedence(
    ...
    monkeypatch.setenv("SIGLIP_V1_WEIGHTS_DIR", str(env_dir))
    resolved, source = model_module.resolve_weights_path()
    assert resolved == env_dir
    assert source == "env_var"

    monkeypatch.delenv("SIGLIP_V1_WEIGHTS_DIR", raising=False)
    ...
    fake_pkg_file = tmp_path / "site-packages" / "siglip_v1_pipeline" / "model.py"
