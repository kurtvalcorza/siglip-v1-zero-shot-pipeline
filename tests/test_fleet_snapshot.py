...
from siglip_v1_pipeline import model as model_mod
...
def test_from_pretrained_weights_dir_stages_verifies_and_loads_the_explicit_path(
    ...
    import siglip_v1_pipeline.pipeline as pipeline_mod
    ...
    pipe = SiglipPipeline.from_pretrained(device="cpu", weights_dir=pinned_to_stand_in)
    ...
    with pytest.raises(FileNotFoundError, match="allow_download=True"):
        SiglipPipeline.from_pretrained(device="cpu", weights_dir=pinned_to_stand_in)
    with pytest.raises(ValueError, match="not both"):
        SiglipPipeline.from_pretrained(
