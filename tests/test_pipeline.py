...
from siglip_v1_pipeline import SiglipPipeline
...
def _pipeline() -> SiglipPipeline:
    return SiglipPipeline(FakeModel(), FakeProcessor(), device="cpu")
...
def test_siglip_text_is_lowercased_without_changing_returned_label():
    processor = FakeProcessor()
    pipe = SiglipPipeline(FakeModel(), processor, device="cpu")
...
def test_retrieval_tie_breaking_is_deterministic_and_stable():
    ...
    pipe = SiglipPipeline(EqualScoreModel(), FakeProcessor(), device="cpu")
