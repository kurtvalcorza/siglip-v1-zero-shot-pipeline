...
def _siglip_texts(values: Sequence[str]) -> list[str]:
    """Match SigLIP training-time text lowercasing before tokenization."""
...
ARTIFACT_FORMAT = "org.valcorza.siglip-base-patch16-256.adapter.v1"
    ...
    def adapt(
        ...
        *,
        ...
        """Bounded supervised fine-tuning of the vision tower on a validated labelled dataset.
            ...
            attention-pool head train (2 blocks by default: 21,264,384 of 203,202,050 parameters; the text tower,
        ...
        prompts = _siglip_texts(_prompts(class_list, prompt_template, class_names_map))
        with (
            torch.no_grad()
...
def load_pipeline(
    *,
    ...
    return SiglipPipeline.from_pretrained(
        device=device,
        cache_dir=cache_dir,
        weights_path=weights_path,
    )
...
INPUT_SCHEMA: dict[str, Any] = {
    ...
    "top_k": [1, None],
    "preprocessing": (
        "the pinned processor resizes every image to 256x256 and normalizes it; text is tokenized "
